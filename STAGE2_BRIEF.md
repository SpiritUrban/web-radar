...
> **Ревізія 4** — після четвертого впровадження (`SpiritUrban/web-radar`, Tauri v2 + React
> **у Cargo-воркспейсі з трьох крейтів**, npm). Збірки всіх чотирьох платформ пройшли з
> першого разу; увесь витрачений час пішов на **сайт**, і майже весь — на те, що бриф радив
> робити хибно або перевіряти інструментом, який бреше.
>
> Найважливіше:
>
> - **бриф сам радив дію, яка робить збій невиправним.** «Re-run failed jobs» для `deploy-site`
>   (розділ 4 і розділ 11) додає ще один артефакт `github-pages` у той самий ран, і кожна
>   наступна спроба гіршає. Правило 14c;
> - **сигнал перевірки з розділу 5 кроку 2 непридатний**: `/repos/O/R/pages` віддає 404 без
>   авторизації навіть коли Pages увімкнено. Я побудував на цьому діагноз і помилився —
>   правило 23a. Це той самий клас, що розділ 9, тільки бреше не перевірка продукту, а
>   **перевірка інфраструктури**;
> - **повідомлення дії може називати не ту причину**: `deploy-pages` під час аварії GitHub
>   каже «Ensure GITHUB_TOKEN has permission id-token: write», хоча права правильні, а
>   справжня причина рядком вище — `503` від сервісу OIDC. Правило 21c;
> - **Cargo-воркспейс зсуває половину шляхів** із розділів 6 і 27a і додає версію в лок на
>   кожен крейт. Правила 37–40.
...
14c. **«Re-run failed jobs» на джобі деплою Pages не лікує, а добиває.** Це пряме виправлення
    поради з розділів 4 і 11 — вона хибна саме для цієї джоби.

    Механіка: повторний запуск виконується **в тому самому рані**, а `upload-pages-artifact`
    завантажує новий артефакт, не прибираючи попередній. Другий `deploy-pages` бачить два
    артефакти з іменем `github-pages` і відмовляється вгадувати:
    ...
    Три властивості роблять це дорогим: перша спроба зазвичай падає з **чужої** причини
    (аварія платформи, правило 21c), очевидна реакція — перезапустити, і кожна реакція
    віддаляє від виправлення. Стан невиправний доти, доки артефакти не видалити руками.

    Лікується кроком **перед** завантаженням; він потребує `actions: write` — і у викликаному
    воркфлоу, і в джобі викликача, бо викликаний не може мати більше прав, ніж йому дали:

    ```yaml
    - name: Remove page artifacts left by earlier attempts
      env:
      ...
      run: |
        ...
        for id in $stale; do gh api -X DELETE "repos/${{ github.repository }}/actions/artifacts/$id" || true; done
    ```

    **Поки цього кроку немає в лозі, з якого зроблено тег** (а перезапуск бере воркфлоу станом
    на реф, що його запустив), єдиний робочий обхід — **окремий запуск `pages.yml`** через
    `workflow_dispatch`: свіжий ран, рівно один артефакт. Версія на сайті від цього не постраждає:
    запуск не з тега резолвить `releases/latest`, а це і є щойно опублікований реліз.
...
21c. **Повідомлення дії може називати не ту причину. Читайте рядок ВИЩЕ за пораду.**
    `actions/deploy-pages` під час аварії платформи завершує так:
    ...
    Останній рядок — не діагноз, а стандартна порада, яку дія друкує на **будь-яку** невдачу з
    токеном. Права при цьому були оголошені правильно і в `pages.yml`, і в джобі викликача; я
    перевірив це, витягнувши файли з тега, і все одно спершу пішов шукати неіснуючий баг.

    **Перше, що робити з падінням у пайплайні, який учора був зелений, — питати не код, а
    платформу.** Один curl, десять секунд, і він же дає докази для звіту:

    ```bash
    curl -s https://www.githubstatus.com/api/v2/status.json | grep -o '"description":"[^"]*"'
    curl -s https://www.githubstatus.com/api/v2/incidents/unresolved.json
    ```

    Того дня це віддало `Partial System Outage` і критичний інцидент з компонентами
    `Actions, Pages`: «Workflow runs are still failing or delayed in starting». Супутній
    симптом тієї ж аварії — джоба висить у `waiting` з написом **«is waiting for github-pages
    deployment approval»**, хоча правила `required_reviewers` в оточенні немає взагалі
    (перевіряється через `/repos/O/R/environments/github-pages`). Схвалювати нема чого — це
    деградований контрол-плейн так виглядає.

    Практичний наслідок для воркфлоу: одна повторна спроба деплою через ~60 с ловить короткий
    503, а анотація на падінні обох має першим рядком казати **що саме НЕ постраждало** —
    інсталятори й `latest.json` живуть у Releases, а не на Pages.
23a. **Перевірочна команда теж мусить бути перевірена — інакше діагноз бреше так само
    впевнено, як зелений тест.** Це розділ 9, повернутий на інструменти, якими ви дивитесь на
    **інфраструктуру**.

    Конкретно: сигнал із розділу 5 кроку 2 — «`/repos/<OWNER>/<REPO>/pages` теж 404» —
    **непридатний**. Цей ендпоінт віддає 404 неавторизованому клієнту навіть коли репозиторій
    публічний, а Pages увімкнено. Я прочитав 404, оголосив «джерело Pages не налаштоване» і
    відправив власника міняти правильне налаштування. Причина була зовсім інша (21c).

    Публічні сигнали, які справді розрізняють стани:

    | питання | чим перевіряти без токена |
    |---|---|
    | Pages узагалі увімкнено? | `has_pages` у `/repos/<OWNER>/<REPO>` |
    | деплої доходять? | `/repos/<OWNER>/<REPO>/deployments` + `statuses_url` кожного |
    | що заважає деплою? | `/repos/<OWNER>/<REPO>/environments/github-pages` — `protection_rules` |
    | сайт живий? | сам URL; «Site not found» ≠ «стара версія» |

    Загальне правило: **перш ніж вірити перевірці, переконайтесь, що вона вміє розрізнити
    обидва стани.** 404, який означає і «немає», і «не покажу» — це не перевірка.
...
27a. **`Swatinem/rust-cache` повертає `target/` — разом із тим, що ви не хотіли повертати.**
    ...
    **У Cargo-воркспейсі ці шляхи інші** — тека збірки одна на весь воркспейс, у корені:
    `target/*/bundle`, `target/*/build/<crate>-*`, а `rust-cache` отримує `workspaces: .`.
    Скопійований дослівно варіант вище нічого не видаляє і **мовчить** про це (правило 38).
...
> справний; треба виправити причину й перезапустити деплой.
>
> **Але перезапускати саме `deploy-site` можна лише якщо у воркфлоу вже є видалення старих
> артефактів (правило 14c).** Без нього кожна спроба додає ще один `github-pages` у той самий
> ран, і після другої деплой стає невиправним у принципі. Тоді замість перезапуску —
> `Actions → Deploy GitHub Pages → Run workflow`: свіжий ран, один артефакт, а маніфест
> резолвить `releases/latest`, тобто щойно опублікований реліз.
>
> Коли крок очищення на місці, працює й звичне
...
## 12а. Cargo-воркспейс замість одного крейта

Бриф написаний для одного пакета або pnpm-монорепо. Воркспейс із кількох крейтів
(`crates/core`, `crates/cli`, `src-tauri`) — природна форма для «CLI + десктоп на спільному
рушії», і він зсуває більше речей, ніж здається.

37. **Версія у `Cargo.lock` лежить окремим рядком на КОЖЕН крейт воркспейсу.**
    `sync-version.mjs` мусить пропатчити всі, а `check-version.mjs` — **звірити всі, а не
    перший знайдений**. Різниця не теоретична: у цьому проєкті перевірка з першого ж запуску
    впала на `web-radar-desktop: 0.2.0`, поки решта вже була `0.3.0`. Перевірка, що читає
    перший збіг регексом, показала б зелене.

    Це та сама вимога, що правило 33: **асертити кількість знайденого**, а не сам факт.

38. **Шляхи `target/`, `--manifest-path` і `workspaces:` зсуваються в корінь.**
    Таблиця перекладу з розділу 6 (`--manifest-path <DESKTOP_DIR>/src-tauri/Cargo.toml`)
    у воркспейсі стає просто `--workspace`:

    | один пакет | воркспейс |
    |---|---|
    | `cargo clippy --manifest-path src-tauri/Cargo.toml` | `cargo clippy --workspace --all-targets` |
    | `rust-cache` `workspaces: src-tauri` | `workspaces: .` |
    | `rm -rf src-tauri/target/*/bundle` | `rm -rf target/*/bundle` (правило 27a) |
    | `src-tauri/Cargo.lock` | один лок у корені — другий видалити з git |

    Найпідступніший тут — `rm`: неправильний шлях не помиляється, він просто нічого не
    видаляє, і правило 27a тихо перестає діяти.

39. **Платформозалежний npm-пакет у `devDependencies` кладе `npm ci` на чужому раннері.**
    У проєкті лежав закріплений `@tauri-apps/cli-win32-x64-msvc` — на Windows усе працює, на
    Linux- і macOS-раннерах `npm ci` падає ще до збірки. Такі пакети ставляться самі як
    optional-залежності основного CLI; у `package.json` їм не місце.

40. **Плагін Tauri — це чотири місця, і забути можна будь-яке.** npm-пакет, крейт у
    `Cargo.toml`, `.plugin(...)` у `lib.rs` і дозвіл у `capabilities/*.json`.

    Якщо є дозвіл, але немає крейта, падає **build script**, і повідомлення виглядає так:

    ```text
    Permission process:allow-restart not found, expected one of clipboard-manager:default,
    clipboard-manager:allow-clear, … (далі ~400 дозволів у один рядок)
    ```

    Причина — перші чотири слова, далі стіна тексту. Правило 21a («різати відомий шум») варто
    застосовувати і до читання таких помилок: шукайте перший рядок, а не гортайте вивід.

41. **Tailwind: `@apply group` заборонений.** `group` — маркер для селекторів нащадків, його
    не можна вносити у власний клас через `@apply`; збірка падає з
    `@apply should not be used with the 'group' utility`. Клас лишається у власному класі,
    `group` дописується в розмітці поруч із ним.
...
- [ ] **кеш `target/` зачищається** від бандлів і від build-виходу крейта перед збіркою (27a),
      і шлях перевірено на реальній структурі проєкту — у воркспейсі він у корені (38)
- [ ] **іконка перевірена в опублікованому інсталяторі**, а не в конфізі (правило 24a)
- [ ] **`pages.yml` видаляє старі артефакти `github-pages` перед завантаженням**, і `actions: write`
      є і у ньому, і в джобі викликача (правило 14c) — інакше перший же перезапуск деплою
      робить ситуацію невиправною
- [ ] **деплой має одну повторну спробу**, а анотація на падінні першим рядком каже, що реліз
      не постраждав (правило 21c)
- [ ] у воркспейсі: `check-version` звіряє **всі** крейти в `Cargo.lock`, а не перший (37)
- [ ] у `package.json` немає платформозалежних пакетів на кшталт `@tauri-apps/cli-win32-x64-msvc` (39)
- [ ] кожен плагін Tauri присутній у **всіх чотирьох** місцях: npm, `Cargo.toml`, `lib.rs`,
      `capabilities` (40)
