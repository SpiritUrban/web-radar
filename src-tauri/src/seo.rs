...
fn default_results_per_query() -> u8 {
    10
}
...
pub fn build_queries(target: &SeoTarget) -> Vec<SeoQuery> {
    let domain = target
        ...
        .trim_end_matches('/')
        .to_lowercase();
    let brand = if target.brand.trim().is_empty() {
        derive_brand(&domain)
    } else {
        target.brand.trim().to_string()
    };
    let rows = [
        (
            "potential_backlink",
            format!("\"{domain}\" -site:{domain}"),
            "Pages mentioning the exact domain outside the target site",
        ),
        (
            "brand_mention",
            format!("\"{brand}\" -site:{domain}"),
            "Unlinked and linked brand mentions",
        ),
        (
            "url_mention",
            format!("\"https://{domain}\" -site:{domain}"),
            "Pages containing the canonical URL",
        ),
        (
            "inpage_domain",
            format!("inpage:\"{domain}\" NOT site:{domain}"),
            "Domain text found in page content",
        ),
        (
            "reputation",
            format!("\"{brand}\" (review OR reviews OR відгуки OR скарги OR scam) -site:{domain}"),
            "Reviews, complaints and reputation signals",
        ),
        (
            "documents",
            format!("\"{domain}\" (filetype:pdf OR filetype:doc OR filetype:xls) -site:{domain}"),
            "Documents and citations mentioning the domain",
        ),
    ];
    rows.into_iter()
        .map(|(kind, query, purpose)| SeoQuery {
            kind: kind.into(),
            query,
...
pub async fn discover(request: SeoDiscoveryRequest) -> Result<SeoDiscoveryReport> {
    ...
    if request.provider == "google" && request.google_cx.trim().is_empty() {
        bail!("Google Search Engine ID (cx) is required");
    }
    if request.provider != "brave" && request.provider != "google" {
        bail!("Unsupported provider: {}", request.provider);
    }
    ...
    for target in &request.targets {
        for query in build_queries(target) {
            eprintln!(
                "[web-radar][seo] provider={} target={} kind={} query={}",
                request.provider, target.domain, query.kind, query.query
            );
            let items: Vec<(String, String, String)> = if request.provider == "brave" {
                ...
                response
                    ...
                    .unwrap_or_default()
                    .into_iter()
                    .map(|x| (x.title, x.url, x.description))
                    .collect()
            } else {
                let response = client
                    ...
                    .error_for_status()
                    .context("Google Custom Search returned an error")?
                    .json::<GoogleEnvelope>()
                    .await
                    .context("Invalid Google Custom Search response")?;
                response
                    .items
                    .into_iter()
                    .map(|x| (x.title, x.link, x.snippet))
                    .collect()
            };
            ...
            for (title, url, snippet) in items {
                ...
                evidence.push(SeoEvidence {
                    target_domain: target_host.into(),
    ...
    let unique_source_domains = evidence
        .iter()
        .map(|e| e.source_domain.as_str())
        .collect::<HashSet<_>>()
        .len();
    let potential_backlinks = evidence
        .iter()
        .filter(|e| e.evidence_type == "potential_backlink")
        .count();
    let brand_mentions = evidence
        .iter()
        .filter(|e| e.evidence_type == "brand_mention")
        .count();
    let reputation_mentions = evidence
        .iter()
        .filter(|e| e.evidence_type == "reputation_mention")
        .count();
    let mut by_domain = BTreeMap::<String, usize>::new();
    for item in &evidence {
        *by_domain.entry(item.source_domain.clone()).or_default() += 1;
    }
    let strongest = by_domain
        .iter()
    ...
    if potential_backlinks > 0 {
        insights.push(SeoInsight { level: "positive".into(), title: "Backlink prospects found".into(), detail: format!("{potential_backlinks} external pages are candidates for manual or automated link verification.") });
    }
    if reputation_mentions > 0 {
        insights.push(SeoInsight { level: "warning".into(), title: "Reputation results need review".into(), detail: format!("{reputation_mentions} review/complaint-oriented results were found; sentiment is not inferred from snippets alone.") });
    }
    if let Some(strongest) = strongest {
        insights.push(SeoInsight {
            level: "info".into(),
            title: "Most visible source".into(),
            detail: format!("The most frequently discovered external domain is {strongest}."),
        });
    }
...
mod tests {
    ...
    fn builds_safe_discovery_queries() {
        let queries = build_queries(&SeoTarget {
            domain: "my-transfer.com.ua".into(),
            brand: "My Transfer".into(),
        });
        assert!(queries
            .iter()
            .all(|q| q.query.contains("-site:my-transfer.com.ua")
                || q.query.contains("NOT site:my-transfer.com.ua")));
        assert!(queries
            .iter()
            .any(|q| q.query == "\"My Transfer\" -site:my-transfer.com.ua"));
